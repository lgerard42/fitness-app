...
**Columns with dependencies:** \`parent_ids\` is a self-referencing list of muscle IDs that define the hierarchy. \`upper_lower\` is used for filtering (e.g. “Upper Body” vs “Lower Body”) in the exercise and motion pickers.`,
  ...
  motions: `**What this table is for:** Motions describe how the body moves during an exercise (e.g. Press, Curl, Squat). Rows can be parent motions (e.g. Press) or variations (e.g. Flat Press, Incline Press) linked by \`parent_id\`. Each motion has muscle targets and a default motion plane; which planes apply to a motion is defined by the Motion Paths table’s \`delta_rules\` (keyed by motion ID).

**Depends on:** \`parent_id\` references this same table (parent motion). \`default_delta_configs\` is a JSON object keyed by table name (e.g. \`motionPaths\`) storing the default ID for that table—e.g. \`{ "motionPaths": "MID_MID" }\` for the default plane.

**Other tables that depend on this one:** Exercises in the main app reference a primary motion and optionally a variation. Motion Paths reference motions via \`delta_rules\` keys. Scoring uses motion + plane + muscle targets.

**Columns with dependencies:** \`parent_id\` → Motions (self). \`default_delta_configs.motionPaths\` → Motion Paths (by ID). \`muscle_targets\` is a JSON tree keyed by muscle group/muscle IDs (conceptually references Muscles).`,

  motionPaths: `**What this table is for:** Motion Paths describe the path or angle of movement (e.g. Low to High, Standard Level). Which motions use which planes is defined here: \`delta_rules\` is keyed by motion ID. Each motion’s default plane is stored in Motions.\`default_delta_configs.motionPaths\`.

**Depends on:** Nothing—this table is standalone.

**Other tables that depend on this one:** Motions store a default plane in \`default_delta_configs.motionPaths\`; the relationship (which planes apply to which motion) is defined by this table’s \`delta_rules\` keys. When a user picks a motion and plane for an exercise, the app uses these IDs.

**Columns with dependencies:** None. \`id\` is referenced from Motions.\`default_delta_configs.motionPaths\`. \`delta_rules\` keys are motion IDs; the scoring system uses them to adjust muscle scores per motion type.`,
