...
- **Structure:** One table; each row has `motion_id` (FK to `motions`), `trigger_conditions_json` (AND conditions: tableKey + operator [`eq` | `in` | `not_eq` | `not_in`] + value), `action_type` (SWITCH_MOTION | REPLACE_DELTA | CLAMP_MUSCLE), `action_payload_json` (shape depends on action type), `priority`, `sort_order`, `is_active`, `label`, `expected_primary_muscles` / `expected_not_primary` (for validation), `notes`.
...
- **Validation:** All scoring JSON should be validated for shape (flat maps, correct keys) and referential integrity (IDs exist in `muscles` / `motions` / modifier tables). Combo rules are validated by `comboRuleValidator` (structural: trigger condition operators [`eq` | `in` | `not_eq` | `not_in`], action payload shape per action type) and by the linter (GET `/api/admin/scoring/lint`): the linter checks unknown motion IDs, SWITCH_MOTION proxy existence and muscle_targets (umbrella warning), REPLACE_DELTA table_key validity **and** row_id existence in the modifier table, CLAMP_MUSCLE muscle existence **and** `is_scorable` (warns if the clamped muscle is not scorable since the clamp will have no effect), unknown action types, and empty conditions. Trace and compute responses include `effectiveMotionId` and `rulesFired` (fixed schema: ruleId, actionType, matchedConditions, specificity, priority, winnerReason) for debugging.
...
| **motionPaths** | No | **JSON object**: `{ "config": <TableConfig>, "deltas": <optional per-row deltas> }`. See below for `config` shape. | **`config`** is written to the Matrix config’s **`config_json.tables.motionPaths`** (postgres: `motion_matrix_configs` row, `config_json` JSONB). `deltas` is used for export round-trip; on import only `config` is applied. |
