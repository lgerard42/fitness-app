...
| **motions / muscles row flags** | Both tables include booleans `is_scorable`, `is_default`, `is_advanced` (defaults: true, true, false). Editable in the Admin table editor side-panel. **is_scorable:** when false, the muscle/motion cannot be assigned a score in any admin muscle tree builder (muscle_targets, delta_rules); the tree still shows it but the score cell is read-only, add-muscle dropdowns exclude it, and its id is never written to `muscle_targets` or `delta_rules`. Used for filtering and display (e.g. default vs advanced options). |
