...
| **combo_rules** | Postgres table (admin key `comboRules`). Rows target a motion and define trigger conditions (AND-match on active modifiers; operators: eq, in, not_eq, not_in) and an action: SWITCH_MOTION, REPLACE_DELTA, or CLAMP_MUSCLE. **Not** in `MODIFIER_TABLE_KEYS`; resolution is order-independent. Linter validates REPLACE_DELTA row existence and CLAMP_MUSCLE is_scorable. Used when the additive delta model is wrong for specific modifier combos. See background_ScoringSystem.md §3.4. |
