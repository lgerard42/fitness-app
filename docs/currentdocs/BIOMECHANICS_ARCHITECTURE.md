...
This order is implemented in `shared/types/matrixV2.ts` as `MODIFIER_TABLE_KEYS`. If the actual engine uses a different order, **update this section to match implementation**. Do not leave a doc-only order.
