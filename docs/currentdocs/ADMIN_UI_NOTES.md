...
- **Combo Rules table (`comboRules`):** Rows have `trigger_conditions_json` (array of `{ tableKey, operator, value }` where operator is `eq` | `in` | `not_eq` | `not_in`) and `action_payload_json` (object; shape depends on `action_type`: SWITCH_MOTION, REPLACE_DELTA, CLAMP_MUSCLE). In the **Combo Rules** tab (Motion Delta Matrix), the modal editor uses textareas with `JSON.stringify(..., null, 2)` for display and `JSON.parse` on save; tolerate both string and parsed object from the API per the rule above.
...
- **Linter:** Validates `default_delta_configs` row IDs (table key and row ID must exist). Combo rules are linted when passed to `lintAll` (loaded by the scoring route): checks include unknown motion IDs, SWITCH_MOTION proxy existence and umbrella warning, REPLACE_DELTA table_key validity **and** row_id existence in the modifier table, CLAMP_MUSCLE muscle existence **and** `is_scorable` check (warns if clamped muscle is not scorable), unknown action types, and empty trigger conditions.
